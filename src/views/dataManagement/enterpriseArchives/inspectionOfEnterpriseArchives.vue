<!-- 企业数据 -->
<template>
    <div class="inspectionOfEnterpriseArchives">
        <breadcrumb>
            <!-- <label for="">省份:</label>
        ...
        <div class="table">
            <el-table
                ...
                label="等级">
                    <template slot-scope="scope">
                        {{scope.row.enterpriseBusinessCategory}}维修经营业务
                    </template>
                </el-table-column> -->
                <!-- <el-table-column
                ...
                label="经营类别">
                    <template slot-scope="scope">
                        <span v-if="scope.row.enterpriseBusinessCategory==='01'">一类维修经营业务</span>
                        <span v-if="scope.row.enterpriseBusinessCategory==='02'">二类维修经营业务</span>
                        <span v-if="scope.row.enterpriseBusinessCategory==='03'">三类维修经营业务</span>
                    </template>
                </el-table-column>
                <el-table-column
                prop="createTime"
...
export default {
    ...
    methods: {
        ...
        getDataList () {
            ...
            this.$post('insEnt/list?page=' + this.pageNum + '&size=' + this.pageSize, obj)
                .then(res => {
                    if (res && res.code === 0) {
        ...
        goDetail (id) {
            // 权限校验**********************************************************
            if (!this.$checkAuth('insEnt:detail')) {
                this.$msg({
                    type: 'error',
                    message: '你没有该项权限'
                })
            ...
            this.$router.push(`/inspectionOfEnterpriseDetail?id=${id}`)
        },
        search () {
            this.pageNum = 1
            this.pageSize = 10
        ...
        exportTable () {
            ...
            //     return
            // }
            window.open(`api/enterprise/export?enterpriseName=${this.enterpriseNamekey}&enterpriseTypeId=${this.enterpriseType}`)
        }
    },
    filters: {
...
.inspectionOfEnterpriseArchives {
    .table {
        margin: 20px;
        margin-top: 0;
    }
