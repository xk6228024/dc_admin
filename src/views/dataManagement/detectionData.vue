    ...
    <div class="detectionData">
        ...
        <div class="table">
            <el-table
                ...
                label="操作">
                <template slot-scope="scope">
                    <el-button @click="goDetail(scope.row.inspectId)" type="text" size="small">详情</el-button>
                </template>
                </el-table-column>
            </el-table>
            <el-pagination
...
export default {
    ...
    methods: {
        // 获取所有数据
        getDataList () {
            // if (!this.$checkAuth('inspect:find')) {
            //     this.$alert('您没有该权限')
            //     return
            // }
            this.loading = true
            ...
            this.$post('inspect/find?page=' + this.pageNum + '&size=' + this.pageSize, obj)
                .then(res => {
                    if (res && res.code === 0) {
