        ...
        <el-form class="form clearfix" ref="form" :model="value" :inline-message='false' :inline='true' v-if="hasBtn" :style="{'width':hasTitle&&'auto','float':hasTitle&&'right'}">
            <div v-for="(item, index) in listOptions" :key="index" class="search_item" >
                <el-form-item  :label="`${item.label}：`" :prop="item.key">
                    <el-input v-if="item.type === 'input'" v-model="value[item.key]"  :disabled="isdisabled" size="mini" clearable placeholder="请输入"></el-input>
                    <el-select v-else-if="item.type === 'select' && item.list"  :disabled="isdisabled" size="mini" v-model="value[item.key]" placeholder="请选择" clearable>
                        <el-option
                    ...
                    <el-select v-else-if="item.type === 'city'"  :disabled="isdisabled" size="mini" v-model="value[item.key]" placeholder="请选择" clearable>
                        <el-option
                    ...
                    <el-button type="" :disabled="getDisabled" size="mini" @click="query" v-if="hasBtn&&!isdisabled">查询</el-button>
                    <el-button @click="reset" size="mini" v-if="hasBtn&&!isdisabled">清空</el-button>
                    <el-button @click="exportFuc" size="mini" v-if="hasBtn&&hasExport">导出</el-button>
            ...
            <el-drawer
                ...
                <div class="drawWrap">
                    <div>
                        <div v-for="(item, index) in options" :key="index" class="search_item" >
                            <el-form-item  :label="`${item.label}：`" :prop="item.key" label-width="350" :inline='false'>
                                <el-input v-if="item.type === 'input'"  :disabled="isdisabled" v-model="value[item.key]"  size="mini" class="w350" clearable placeholder="请输入"></el-input>
                                <el-select v-else-if="item.type === 'select'  && item.list" size="mini" v-model="value[item.key]"  :disabled="isdisabled" placeholder="请选择" class="w350" clearable>
                                    <el-option
                                    v-for="child in item.list"
                                    :key="child[item.list_value] || child.value"
                                    :label="child[item.list_key] || child.label"
                                ...
                                <el-select v-else-if="item.type === 'city'" size="mini" v-model="value[item.key]" placeholder="请选择"  :disabled="isdisabled" class="w350" clearable>
                                    <el-option
                                    v-for="child in regionList"
                                    :key="child.regionId"
                                    :label="child.regionName"
                    ...
                    <div class="btnBottom">
                        <el-button class="fl" type="warning" @click="query" style="background-color:#998B6F;"  :disabled="isdisabled">查询</el-button>
                        <el-button class="fr" @click="exportFuc" v-if="hasExport"  :disabled="isdisabled">导出</el-button>
                        <el-button class="fr" @click="reset"  :disabled="isdisabled">清空</el-button>
                    </div>
                </div>
            </el-drawer>
        </el-form>
...
export default {
    ...
    methods: {
        ...
        exportFuc () {
            this.$emit('exportFuc', this.value)
        },
        reset () {
