...
const ifShow = process.env.VUE_APP_CURRENTMODE === 'show'
...
axios.interceptors.response.use(function (response) {
    if (/^1003$/.test(response.data.code) && process.env.VUE_APP_MODE !== 'show') {
        clearCookie('token')
        clearCookie('user')
...
function post (url, params, config) {
    if (process.env.VUE_APP_MODE === 'show') {
        return new Promise((resolve, reject) => { resolve({ code: 0, data: [{}] }) })
    }
    config = config || {}
