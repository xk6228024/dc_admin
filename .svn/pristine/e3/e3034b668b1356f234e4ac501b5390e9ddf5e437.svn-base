...
<template>
    <div class="carDataStatistics">
        ...
        <div class="right_div" v-loading="loading">
            ...
            <div class="right_header" >车辆数据统计
                <!-- <span v-if="type==1">车辆总计: <i>{{allCount}}辆</i></span> -->
            </div>
        ...
        <div class="bottom_div">
            ...
            <el-radio v-model="radio" label="brand">按品牌统计</el-radio>
...
export default {
    ...
    created () {
        this.getData()
        this.$post('statistics/vehicle/all').then(res => {
            console.log(res)
            this.allCount = res.count
            // this.option.title.text = res.count + '辆'
        })
        setTimeout(() => {
            this.showTips = false
        }, 3000)
    ...
    mounted () {
        ...
        if (clientWidth < 1200) {
            this.$refs.chartOfStatistics.style.width = '650px'
        } else {
    ...
    methods: {
        ...
        getData () {
            this.loading = true
            this.$post(`/statistics/vehicle/${this.radio}`).then(res => {
                this.loading = false
                if (this.radio === 'series') {
                    res.data.forEach(e => {
                        if (e.name && e.name.indexOf('/') !== -1) {
                            // return { name: e.name.split('/')[1], count: e.count }
                            e.name2 = e.name.split('/')[1]
                            // console.log(e.name2)
                        } else {
                            e.name2 = '--'
                            // return { name: '未知', count: e.count }
                        }
                    })
                }
